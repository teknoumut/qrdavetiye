<x-app-layout>
    <div class="max-w-lg mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        <div class="bg-white dark:bg-night-800 rounded-2xl border border-cream-200 dark:border-night-700 p-8 sm:p-10 shadow-sm">
            ...
            <h1 class="text-2xl font-extrabold text-night-900 dark:text-cream-100 mb-2">{{ __('Ödeme Bildirimi Alındı!') }}</h1>
            <p class="text-night-400 dark:text-night-500 text-sm mb-6">{{ __('Ödeme bildiriminiz başarıyla iletilmiştir. Aboneliğiniz admin onayından sonra aktifleşecek.') }}</p>
            ...
            <div class="bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 rounded-xl p-4 mb-6 text-left space-y-2">
                <div class="flex items-center justify-between text-sm">
                    <span class="text-night-400 dark:text-night-500">{{ __('Sipariş No') }}</span>
                    <span class="font-mono font-bold text-night-900 dark:text-cream-100">{{ $orderNo }}</span>
                </div>
                <div class="flex items-center justify-between text-sm">
                    <span class="text-night-400 dark:text-night-500">{{ __('Plan') }}</span>
                    <span class="font-semibold text-night-900 dark:text-cream-100">{{ $plan->name }}</span>
                </div>
                <div class="flex items-center justify-between text-sm">
                    <span class="text-night-400 dark:text-night-500">{{ __('Durum') }}</span>
            ...
            <div class="bg-blue-50 dark:bg-blue-500/5 border border-blue-200 dark:border-blue-500/20 rounded-xl p-4 text-xs sm:text-sm text-blue-800 dark:text-blue-300 flex items-start gap-3 mb-6 text-left">
                ...
                <div>
                    <strong>{{ __('Sonraki Adım:') }}</strong> {{ __('Ödemeniz admin tarafından kontrol edilip onaylandığında panelinizde yeşil bir bildirim göreceksiniz. Bu sayfayı kapatabilir, panelinize giriş yaptığınızda durumu görebilirsiniz.') }}
                </div>
            </div>
