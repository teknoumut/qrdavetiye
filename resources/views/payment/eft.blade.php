<x-app-layout>
        ...
        <div class="bg-white dark:bg-night-800 rounded-2xl border border-cream-200 dark:border-night-700 p-6 sm:p-8 shadow-sm mb-6">
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-500/10 dark:to-emerald-500/5 rounded-xl border border-emerald-200 dark:border-emerald-500/20 p-5 mb-6">
                <div class="space-y-1.5 text-sm mb-3">
                    ...
                    @if($taxRate > 0)
                    <div class="flex items-center justify-between text-night-500 dark:text-night-400">
                        <span>{{ __('KDV (:tax%)', ['tax' => number_format($taxRate, 0)]) }}</span>
                ...
                <div class="text-center">
                    <div class="text-3xl font-black text-night-900 dark:text-cream-100">{{ formatCurrency($total) }}</div>
                    <div class="text-sm text-night-400 dark:text-night-500 mt-1">{{ __('KDV Dahil Toplam Tutar') }}</div>
                </div>
            </div>
            ...
            <div class="space-y-4 mb-6">
                @if($bankName)
                <div>
                    <label class="block text-xs font-semibold text-night-400 dark:text-night-500 uppercase tracking-wider mb-1.5">{{ __('Banka') }}</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-night-400 dark:text-night-500 uppercase tracking-wider mb-1.5">{{ __('Alıcı Adı') }}</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-night-400 dark:text-night-500 uppercase tracking-wider mb-1.5">{{ __('Açıklama') }}</label>
                    ...
                    <p class="text-xs text-night-400 dark:text-night-500 mt-1.5">{{ __("EFT/Havale açıklama kısmına yukarıdaki sipariş numarasını yazmayı unutmayın.") }}</p>
                </div>
            </div>
            ...
            <div class="bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 rounded-xl p-4 text-xs sm:text-sm text-amber-800 dark:text-amber-300 flex items-start gap-3 mb-6">
                ...
                <div>
                    <strong>{{ __('Önemli:') }}</strong> {{ __('Havale işleminizi tamamladıktan sonra aşağıdaki "Ödeme Bildir" butonuna tıklayarak bildirim gönderin. Ödemeniz admin tarafından onaylandığında aboneliğiniz aktifleşecektir.') }}
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('payment.eft.notify') }}">
                ...
                <div class="mb-5">
                    ...
                    <textarea name="notes" rows="2" class="w-full px-4 py-3 rounded-xl border border-cream-200 dark:border-night-700 bg-white dark:bg-night-900 text-night-900 dark:text-cream-100 text-sm placeholder:text-night-300 dark:placeholder:text-night-500 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-500/20 outline-none transition-all resize-none" placeholder="{{ __('Ödeme bilgisi veya ek not...') }}"></textarea>
                </div>
    ...
    <script>
        function showCopiedToast() {
            ...
            t.textContent = '{{ __("IBAN kopyalandı!") }}';
