    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="bg-white dark:bg-night-800 rounded-2xl border border-cream-200 dark:border-night-700 p-6 sm:p-8 shadow-sm">
            ...
            <div class="bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 rounded-xl p-4 text-sm text-amber-800 dark:text-amber-300 flex items-start gap-3">
                ...
                <div>
                    <p>{{ __('Bu ödeme yönteminde EFT/Havale kullanılır. Ödeme bildiriminiz admin tarafından onaylandıktan sonra aboneliğiniz aktifleşir.') }}</p>
                    <p class="mt-2 text-xs opacity-80">{{ __('Davetiye oluşturup yayınladıktan sonra iade yapılmaz. Detaylı bilgi için') }} <a href="{{ route('legal.iade') }}" class="underline hover:opacity-100">{{ __('İade Politikası') }}</a>'nı inceleyin.</p>
                </div>
            </div>
        </div>
    </div>
